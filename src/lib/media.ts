...
async function decode(ref: MediaRef, blob: Blob): Promise<LoadedMedia> {
  ...
  const element = ref.kind === "video" ? await loadVideo(url) : await loadImage(url);
...
export function extractFiles(dt: DataTransfer | null): File[] {
  ...
  if (dt.items) {
    ...
    for (const f of Array.from(dt.files)) if (f.type.startsWith("image/") || f.type.startsWith("video/")) files.push(f);
  }
  return files;
}
